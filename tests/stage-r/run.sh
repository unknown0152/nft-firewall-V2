...
if (( contract_status != 0 || release_guard_status != 0 || baseline_status != 0 || comparison_status != 0 || secret_scan_status != 0 )); then
    printf 'STAGE R UNPRIVILEGED SOURCE CONTRACTS: FAIL (contracts=%d release_guard=%d baseline=%d comparison=%d secret_scan=%d)\n' \
        "$contract_status" "$release_guard_status" "$baseline_status" "$comparison_status" "$secret_scan_status" >&2
