...
| Privileged R2 | FAIL, HARD STOPPED | The latest approved run reached managed first setup, where publishing the journal before discovery caused correct self-classification as existing NFTFW state and left rollback without a prepared plan. A renewed run is NOT EXECUTED and requires new identity-bound approval |
